...
提供商列表右上角的「批量管理」图标（列表带勾选线）可进入多选模式：

1. 点击左侧列表项即勾选或取消勾选，不影响当前正在查看的提供商详情。
2. 底部操作栏支持「全选当前列表」「启用」「禁用」「完成」。
3. 「全选当前列表」严格遵循搜索范围；修改搜索词会清空已有选择，避免误操作到已隐藏的提供商。
4. 批量禁用前会二次确认，仅切换启用状态，**不会删除**提供商的密钥、模型及其他配置。

批量操作在 SQLite 中以单事务原子写入：要么全部生效，要么整体回滚重试；状态本就一致的提供商会跳过写入，并提示实际变更数量。大批量调整后建议重启 Pi 编码智能体，以确保新配置被加载。
